...
class PreciseTemporalAlignment:
    ...
    def __init__(
        ...
        config = PreciseEngineConfig(
            max_resolutions=4,
            base_resolution=16,
            drift_correction_interval=100
        )
        self.multi_res_aligner = MultiResolutionAligner(
            fingerprinter, config, verbose
        )
    ...
    def refine_with_sliding_window(
        ...
        for w in range(num_windows):
            ...
            for offset in range(min(search_range * 2, len(bg_window) - len(fg_window) + 1)):
                score = 0
    ...
    def align(
        ...
        combined_mapping = (
            0.5 * multi_res_mapping +
            0.5 * full_bidir_mapping
        ).astype(int)
