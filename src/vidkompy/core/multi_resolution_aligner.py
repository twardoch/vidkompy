...
class MultiResolutionAligner:
    ...
    def __init__(
        ...
        config: Optional[PreciseEngineConfig] = None,
        verbose: bool = False
    ):
    ...
    def hierarchical_alignment(
        ...
        for next_res in sorted(self.resolutions, reverse=True)[1:]:
            mapping = self.refine_alignment(
                fg_pyramid, bg_pyramid,
                mapping, current_res, next_res
            )
    ...
    def align(
        ...
        fg_fingerprints: Optional[np.ndarray] = None,
        bg_fingerprints: Optional[np.ndarray] = None
        ...
        """
        logger.info(f"Multi-resolution alignment: {len(fg_frames)} -> {len(bg_frames)} frames")
